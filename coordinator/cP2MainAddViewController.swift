//
//  cP2MainAddViewController.swift
//  trackPro
//
...
import UIKit
import Firebase
class cP2MainAddViewController: UIViewController,UITextFieldDelegate,UIPopoverPresentationControllerDelegate,getval_spl,getval_spl2 {
        func getSelectedBatch(selectedBatch: String) {
          currBatch = Int(selectedBatch)!
           fetchBatchData()

           cP2AddBatch.setTitle(selectedBatch, for: .normal)
       }
       func getSelectedUsn(selectedUsn: String) {
        self.submit.isHidden = false
            currUsn = selectedUsn
        validateUsnData()
            cP2AddUsn.setTitle(selectedUsn, for: .normal)
        updateProgress{() -> () in
            print("hello") }
       }
    var currBatch:Int = 0
    ...
    @IBOutlet var cP2AddBatch: UIButton!
    
    @IBOutlet var cP2AddUsn: UIButton!
    ...
    @IBAction func cP2AddBatchFunc(_ sender: UIButton) {
    ...
    @IBAction func cP2AddUsnFunc(_ sender: UIButton) {
        ...
        var sum = Int(novelty.text!)! + Int(implementationandcoding.text!)! + Int(resultandperformance.text!)!
                     var sum2 = sum + Int(presentation.text!)! + Int(vivaVoice.text!)! + Int(report.text!)! + Int(publication.text!)! +
                      ...
                      let db = Database.database().reference()
                       let branch = db.child("CoordinatorMarks").child("Phase2")
    ...
    func validateUsnData()
        {
            ...
            fetchDetails().getCoordPhase2Marks { (usnData, error) in
